...
def create_mask_transparent_background(input_dir, output_dir, tb_use_fast_mode, tb_use_jit, st1_mask_threshold):
    ...
    jit_str = " --jit" if tb_use_jit else ""
    bin_path = os.path.join("venv", "Scripts")
    # note the "exe" addition
    bin_path = os.path.join(bin_path, "transparent-background.exe")
    print("--> Bin = " + bin_path)
    ...
    a1111_dir = os.path.abspath(os.path.abspath(
    ...
    cmd_out = py_dir + " " + exe_dir + " --source " + input_dir + \
        " --dest " + output_dir + " --type map" + fast_str + jit_str
